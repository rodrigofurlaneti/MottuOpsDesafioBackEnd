...
namespace MottuOpsDesafioBackEnd.Test.UnitTest.Domain.Models
{
    public class MotorcycleRentalModelTests
    {
        ...
        public void MotorcycleRentalModel_Should_Have_Default_Values()
        {
            ...
            Assert.Equal(default, rental.StartDate);
            Assert.Equal(default, rental.EndDate);
            Assert.Equal(default, rental.ExpectedEndDate);
            Assert.Equal(0m, rental.DailyRate);
            Assert.Equal(default, rental.CreatedAt);
